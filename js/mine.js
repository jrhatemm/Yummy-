...
function openNav() {
    ...
    for (let i = 0; i < 5; i++) {
        $(".menu_items li").eq(i).animate({ top: 0 }, (i + 5) * 100)
    }
}

function closeNav() {
    let tabWidth = $(".navbar_menu .navbar-tab").outerWidth()
    $(".navbar_menu").animate({ left: -tabWidth }, 500)
    $(".open-close-icon").addClass("fa-align-justify");
    $(".open-close-icon").removeClass("fa-x");
    $(".menu_items li").animate({ top: 300 }, 500)
}
closeNav()
$(".navbar_menu i.open-close-icon").click(() => {
    if ($(".navbar_menu").css("left") == "0px") {
        closeNav()
    } else {
        openNav()
    }
...
function emailValidation() {
    return (/^[A-Za-z\._\-0-9]*[@][A-Za-z]*[\.][a-z]{2,4}$/.test(document.getElementById("emailInput").value))
}
